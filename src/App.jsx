...
} catch (e) {}};const playChipBeep = (type) => {try {if (!audioCtx) initAudio();if (!audioCtx) return;const osc = audioCtx.createOscillator();const gain = audioCtx.createGain();osc.connect(gain);gain.connect(audioCtx.destination);const now = audioCtx.currentTime;
const vol = 0.02; 
...
};const fetchCoverBySearch = async (item, settings, activeCategories) => {const titleRaw = item.title ? item.title.trim() : '';const authorRaw = item.author_developer ? item.author_developer.trim() : '';const typeRaw = item.type ? item.type.trim() : '';const yearRaw = item.year ? String(item.year).trim() : '';const pubRaw = item.publisher ? item.publisher.trim() : '';const barcodeRaw = item.barcode ? item.barcode.replace(/[-\s]/g, "") : '';const qTitle = encodeURIComponent(titleRaw);const qAuthor = encodeURIComponent(authorRaw);const qPub = encodeURIComponent(pubRaw);const isBook = (activeCategories['Livros'] || []).includes(typeRaw);const isDisc = (activeCategories['Discos'] || []).includes(typeRaw);const isGame = (activeCategories['Games'] || []).includes(typeRaw);const isVideo = (activeCategories['Vídeo'] || []).includes(typeRaw);// 1. Busca por Código de Barras (Mais precisa)if (barcodeRaw) {try {const upcRes = await fetchTimeout(https://api.upcitemdb.com/prod/trial/lookup?upc=${barcodeRaw});const upcData = await upcRes.json();if (upcData.items?.[0]?.images?.[0]) return upcData.items[0].images[0];} catch(e) {} if (isBook) {
    try {
...
}// 2. Busca rigorosa por Metadadosif (isDisc) {if (settings?.discogsToken) {try {let formatQuery = '';const tLower = typeRaw.toLowerCase();if (tLower.includes('vinil') || tLower.includes('lp')) formatQuery = '&format=vinyl';else if (tLower.includes('cd')) formatQuery = '&format=cd';else if (tLower.includes('cassete') || tLower.includes('fita')) formatQuery = '&format=cassette';    let queryUrl = `https://api.discogs.com/database/search?release_title=${qTitle}&artist=${qAuthor}${formatQuery}&token=${settings.discogsToken}`;
...
}else if (isBook) {try {let gbQuery = intitle:"${titleRaw}";if (authorRaw) gbQuery += +inauthor:"${authorRaw}";    let gbRes = await fetchTimeout(`https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(gbQuery)}&maxResults=10`);
...
}else if (isGame) {try {const gameQuery = ${titleRaw} ${typeRaw} game cover;const wikiRes = await fetchTimeout(https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(gameQuery)}&utf8=&format=json&origin=*);const wikiData = await wikiRes.json();      if (wikiData.query?.search?.length > 0) {
...
}else if (isVideo) {try {const itRes = await fetchTimeout(https://itunes.apple.com/search?term=${qTitle}&media=movie&limit=10);const itData = await itRes.json();if (itData.results?.length > 0) {let bestMovie = itData.results[0];if (yearRaw) {const exact = itData.results.find(m => m.releaseDate?.startsWith(yearRaw));if (exact) bestMovie = exact;}if (bestMovie?.artworkUrl100) return bestMovie.artworkUrl100.replace('100x100bb', '600x600bb');}} catch (e) {}}return null;};// ==========================================// ÍCONES NATIVOS SVG// ==========================================const Icon = ({ path, className = "w-6 h-6", onClick, fill = "none", style }) => ({path});const KatamariIcon = ({ className = "w-6 h-6", glow = 0 }) => ( 0 ? drop-shadow(0 0 ${glow}px currentColor) : 'none' }}>);const Search = p => <Icon {...p} path={<></>} />;const Library = p => <Icon {...p} path={<></>} />;const PlusSquare = p => <Icon {...p} path={<></>} />;const BarChart2 = p => <Icon {...p} path={<></>} />;const Settings = p => <Icon {...p} path={<></>} />;const Camera = p => <Icon {...p} path={<></>} />;const Sun = p => <Icon {...p} path={<></>} />;const Download = p => <Icon {...p} path={<></>} />;const Upload = p => <Icon {...p} path={<></>} />;const ExternalLink = p => <Icon {...p} path={<></>} />;const Star = ({ className = '', onClick, style }) => <Icon onClick={onClick} className={className} style={style} fill={className.includes('fill') ? 'currentColor' : 'none'} path={} />;const ChevronLeft = p => <Icon {...p} path={} />;const ChevronRight = p => <Icon {...p} path={} />;const ChevronDown = p => <Icon {...p} path={} />;const ChevronUp = p => <Icon {...p} path={} />;const Check = p => <Icon {...p} path={} />;const ScanLine = p => <Icon {...p} path={<></>} />;const Clock = p => <Icon {...p} path={<></>} />;const Flame = p => <Icon {...p} path={} />;const Ghost = p => <Icon {...p} path={<></>} />;const LibraryBig = p => <Icon {...p} path={<></>} />;const AlertTriangle = p => <Icon {...p} path={<></>} />;const Sparkles = p => <Icon {...p} path={<></>} />;const FilterIcon = p => <Icon {...p} path={<></>} />;const Calendar = p => <Icon {...p} path={<></>} />;const Smartphone = p => <Icon {...p} path={<></>} />;const DiscIcon = p => <Icon {...p} path={<></>} />;const MonitorPlay = p => <Icon {...p} path={<></>} />;const XIcon = p => <Icon {...p} path={<></>} />;const Zap = p => <Icon {...p} path={<></>} />;const ListIcon = p => <Icon {...p} path={<></>} />;const Share = p => <Icon {...p} path={<></>} />;const Headphones = p => <Icon {...p} path={<></>} />;const Music = p => <Icon {...p} path={<></>} />;const ImageIcon = p => <Icon {...p} path={<></>} />;const RefreshIcon = p => <Icon {...p} path={<></>} />;const Trash2 = p => <Icon {...p} path={<></>} />;// ==========================================// PWA ENGINE// ==========================================const usePWA = (iconUrl) => {const [installPrompt, setInstallPrompt] = useState(null);const [isInstalled, setIsInstalled] = useState(false);useEffect(() => {const manifest = {name: "Memorabilia",short_name: "Memorabilia",description: "Sua coleção física na palma da mão.",start_url: ".",display: "standalone",background_color: "#ffffff",theme_color: "#000000",icons: [{ src: iconUrl, sizes: "192x192", type: "image/png", purpose: "any maskable" },{ src: iconUrl, sizes: "512x512", type: "image/png", purpose: "any maskable" }]};const manifestUrl = URL.createObjectURL(new Blob([JSON.stringify(manifest)], { type: 'application/json' }));
...
return 'Mais Antigos';
  ...
  return result;
}, [items, searchTerm, alphaFilter, sortBy]);const filterCounts = useMemo(() => {const counts = { Adicionado: {}, Suporte: {}, Ano: {}, Nota: {}, 'Páginas/Faixa': {} };  baseFilteredItems.forEach(item => {
  ...
  return result;
}, [baseFilteredItems, activeFilters, sortBy, sortOrder]);const handleItemPressStart = (item) => {isLongPress.current = false;pressTimer.current = setTimeout(() => { isLongPress.current = true; setContextMenuItem(item); }, 500);};const handleItemPressEnd = () => { if (pressTimer.current) clearTimeout(pressTimer.current); };const handleItemClick = (item) => { if (!isLongPress.current) handleSelect(item); };const paginatedItems = useMemo(() => {return finalProcessedItems.slice(page * itemsPerPage, (page + 1) * itemsPerPage);}, [finalProcessedItems, page, itemsPerPage]);const totalPages = Math.ceil(finalProcessedItems.length / itemsPerPage) || 1;const alphabetOptions = ['Todos', '#', ...Array.from({length: 26}, (_, i) => String.fromCharCode(65 + i))];const handleSelect = (item) => { setSelectedItem(item); setEditedItem({ ...item }); };const updateRatingList = (id, r) => {const u = { ...items.find(i => i.id === id), rating: r };setItems(items.map(item => item.id === id ? u : item));playChipBeep('save');onShowToast('success');syncItemToSheets(u, settings?.googleSheetsUrl);};const saveModifications = () => {setItems(items.map(i => i.id === editedItem.id ? editedItem : i));setSelectedItem(editedItem);playChipBeep('save');onShowToast('success');syncItemToSheets(editedItem, settings?.googleSheetsUrl);};const handleSearchCover = async () => {setIsSearchingCover(true);try {const newCover = await fetchCoverBySearch(editedItem, settings, activeCategories);if (newCover) {setEditedItem(prev => ({ ...prev, cover_url: newCover }));playChipBeep('success');onShowToast('success');} else {playChipBeep('error');}} catch (e) {playChipBeep('error');} finally {setIsSearchingCover(false);}};const confirmDelete = async () => {if (itemToDelete) {const updatedList = items.filter(item => item.id !== itemToDelete);const reindexedList = reindexCollection(updatedList);   setItems(reindexedList); 
...
};const fetchWikiInfo = async () => {const apiKey = (settings?.geminiApiKey || "").trim();if (!apiKey) { setWikiError("Chave API ausente."); playChipBeep('error'); return; }setLoadingWiki(true); 
setWikiError('');
...
};const countActiveFilters = Object.values(activeFilters).reduce((acc, curr) => acc + curr.length, 0);if (selectedItem && editedItem) {const isBookOrGame = [...(activeCategories['Livros'] || []), ...(activeCategories['Games'] || [])].includes(editedItem.type);const isDiscItem = (activeCategories['Discos'] || []).includes(editedItem.type);const isVideoItem = (activeCategories['Vídeo'] || []).includes(editedItem.type);const linkInfo = getExternalLinkInfo(editedItem.type, editedItem.title);const metricLabel = getMetricInfo(editedItem.type, activeCategories).label;const imageContainerClass = isDiscItem ? "w-40 h-40 md:w-56 md:h-56 aspect-square" : "w-32 h-44 md:w-48 md:h-64 aspect-[3/4]";return (
...
}return (<MModal isOpen={!!itemToDelete} title="Excluir Item" message={Apagar "${editedItem?.title}"?} onConfirm={confirmDelete} onCancel={() => {setItemToDelete(null); setEditedItem(null);}} darkMode={darkMode} confirmText="Apagar" />  {contextMenuItem && (
...
}, [addMode, isHtml5QrcodeLoaded]);const fetchMultiDatabaseParallel = async (barcode) => {const cleanCode = barcode.replace(/[-\s]/g, "").toUpperCase();updateStatus('loading', 'Consultando bancos de dados simultaneamente...');// Melhoria na detecção de ISBN: aceita ISBN-13 (começa com 978/979) e ISBN-10 (10 dígitos)
...
};const handleSave = () => {if (!formData.title) {playChipBeep('error');setShowErrorModal(true);return;}const classCode = activeClassCodes[formData.type] || '000'; 
...
setActiveTab('library');
};const isBookOrGame = [...(activeCategories['Livros'] || []), ...(activeCategories['Games'] || [])].includes(formData.type);const metricInfo = getMetricInfo(formData.type, activeCategories);return (<MModal isOpen={showErrorModal} title="Atenção" message="O Título é obrigatório." onConfirm={() => setShowErrorModal(false)} onCancel={() => setShowErrorModal(false)} darkMode={darkMode} confirmText="OK" cancelText="Fechar" /><MButton darkMode={darkMode} variant={addMode === 'manual' ? 'cyan' : 'white'} onClick={() => changeMode('manual')} className="flex-1 py-2 text-[10px]"> Manual<MButton darkMode={darkMode} variant={addMode === 'barcode' ? 'amber' : 'white'} onClick={() => changeMode('barcode')} className="flex-1 py-2 text-[10px]"> Barcode Auto IA  {displayBoxState !== 'idle' && (
  ...
  {addMode === 'barcode' && (
    <MContainer darkMode={darkMode} className="flex-1 mb-4 flex flex-col relative overflow-hidden bg-black items-center justify-center min-h-[300px]">
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-24 border-[4px] border-white shadow-[0_0_0_9999px_rgba(0,0,0,0.5)] pointer-events-none flex flex-col items-center justify-center z-20"><span className="text-white text-[10px] uppercase font-black tracking-widest bg-black px-3 py-1 mt-20">Alinhe o Código</span></div>
    </MContainer>
  )}
...
);};const DashboardTab = ({ items, darkMode, activeCategories }) => {const [filterCat, setFilterCat] = useState('Todas');const [filterSubtype, setFilterSubtype] = useState('Todos');const [filterStatus, setFilterStatus] = useState('Todos');const [filterRating, setFilterRating] = useState('Todas');const dashItems = useMemo(() => {return items.filter(item => {let mCat = true, mStatus = true, mRating = true;if (filterCat !== 'Todas') {if (filterSubtype !== 'Todos') mCat = item.type === filterSubtype;else mCat = (activeCategories[filterCat] || []).includes(item.type);}if (filterStatus !== 'Todos') {const isDisc = (activeCategories['Discos'] || []).includes(item.type);const isVideo = (activeCategories['Vídeo'] || []).includes(item.type);if (isDisc || isVideo) mStatus = false;else mStatus = item.status === filterStatus;}if (filterRating !== 'Todas') mRating = item.rating === parseInt(filterRating);return mCat && mStatus && mRating;});}, [items, filterCat, filterSubtype, filterStatus, filterRating, activeCategories]);const chartColors = getChartColors(darkMode);const catCounts = {}; const statusCounts = {};dashItems.forEach(item => {let foundCat = 'Outros';for (const [cat, subs] of Object.entries(activeCategories)) { if ((subs || []).includes(item.type)) { foundCat = cat; break; } }catCounts[foundCat] = (catCounts[foundCat] || 0) + 1;const isDisc = (activeCategories['Discos'] || []).includes(item.type);
const isVideo = (activeCategories['Vídeo'] || []).includes(item.type);
if (!isDisc && !isVideo) {
  const st = item.status || 'Não Iniciado';
  statusCounts[st] = (statusCounts[st] || 0) + 1;
}
});const catChartData = Object.entries(catCounts).map(([label, value], idx) => ({ label, value, colorHex: chartColors[idx % chartColors.length] })).sort((a,b) => b.value - a.value);const statusChartData = Object.entries(statusCounts).map(([label, value], idx) => ({ label, value, colorHex: chartColors[(idx + 2) % chartColors.length] })).sort((a,b) => b.value - a.value);const totalDash = dashItems.length;const byType = dashItems.reduce((acc, i) => { acc[i.type || 'Outro'] = (acc[i.type || 'Outro'] || 0) + 1; return acc; }, {});const sortedTypes = Object.entries(byType).sort((a, b) => b[1] - a[1]).slice(0, 5);const maxType = sortedTypes.length > 0 ? sortedTypes[0][1] : 1;const byAuthor = dashItems.reduce((acc, i) => {if (i.author_developer) {const rawAuthor = i.author_developer.trim();if (isVariousArtists(rawAuthor)) return acc;  const normTitle = normalizeWorkTitle(i.title);
...
}, [musicItems, hasMusicStats]);// VIDEO (FILMES)const videoItems = dashItems.filter(i => (activeCategories['Vídeo'] || []).includes(i.type));const hasVideoStats = videoItems.length > 0 && (filterCat === 'Todas' || filterCat === 'Vídeo');const videoStats = useMemo(() => {if (!hasVideoStats) return null;const assistidos = videoItems.filter(i => (Number(i.rating) || 0) > 0);const qtyAssistidos = assistidos.length;const mediaNota = qtyAssistidos > 0 ? (assistidos.reduce((acc, i) => acc + Number(i.rating), 0) / qtyAssistidos).toFixed(1) : 0;const percAssistidos = videoItems.length > 0 ? Math.round((qtyAssistidos / videoItems.length) * 100) : 0; let totalMinutos = 0;
 const minByDirector = {};
...
Analise a imagem enviada. Pode ser uma capa de livro, quadrinho, encarte de CD/Vinil/Fita, caixa de DVD, capa de game ou FICHA CATALOGRÁFICA (expediente).Extraia os metadados com precisão e retorne EXCLUSIVAMENTE um JSON válido, sem crases de markdown.Formato exigido:{"type": "Escolha APENAS uma: ${allTypes.join(', ')}","title": "Título Principal da obra","author_developer": "Autor(es), Artista, Banda ou Desenvolvedora","year": "Ano de lançamento original ou da edição (formato YYYY, ex: 1990)","publisher": "Editora, Gravadora ou Produtora","pages_or_time": "Número de páginas (impressos), faixas (música) ou minutos (filmes)","description": "Breve sinopse identificada ou resumo da obra"}REGRAS RÍGIDAS:Em gibis e revistas (como na imagem de expediente), procure informações miúdas como 'Editora Abril', 'Panini', etc.Datas abreviadas como 'Julho/90' devem ser formatadas apenas como '1990'.O título deve ser o nome principal em destaque (ex: 'Lex Luthor').Retorne APENAS o JSON puro. Nenhuma palavra a mais.`;// Atualizado para o Endpoint robusto mais recente da API Vision (gemini-2.5-flash)const res = await fetch(https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}, {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({contents: [{parts: [{ text: promptInstructions },{ inlineData: { mimeType: "image/jpeg", data: b64 } }]}],generationConfig: { responseMimeType: "application/json" }})});if (!res.ok) {let errMsg = Erro HTTP: ${res.status};try {const errData = await res.json();if (errData.error && errData.error.message) errMsg = errData.error.message;} catch(e) {}throw new Error(errMsg);}const data = await res.json();let text = data.candidates?.[0]?.content?.parts?.[0]?.text;if (!text) throw new Error("Retorno vazio da IA.");text = text.replace(/json/gi, '').replace(//g, '').trim();text = text.substring(text.indexOf('{'), text.lastIndexOf('}') + 1);setScannedAIData(JSON.parse(text));setAiBoxState('success');setAiBoxMessage('Extraído com sucesso da imagem!');playChipBeep('save');showToast('success');} catch (e) {let errorMsg = e.message;if (errorMsg.includes('429') || errorMsg.toLowerCase().includes('quota') || errorMsg.includes('exceeded')) {errorMsg = "Cota gratuita da IA esgotada no momento. ⚠️\nUse o botão 'Barcode' (Código de Barras) para pesquisar bases de dados sem limite, ou preencha manualmente até a cota resetar.";} else {errorMsg = Falha na IA: ${errorMsg}\nTente focar bem o texto do expediente ou use o modo Barcode.;}setAiBoxState('error');setAiBoxMessage(errorMsg);playChipBeep('error');showToast('error');}};useEffect(() => {if (window.Html5Qrcode) { setIsHtml5QrcodeLoaded(true); return; }if (!document.getElementById('html5-qrcode')) {const script = document.createElement('script');script.id = 'html5-qrcode';script.src = "https://unpkg.com/html5-qrcode/html5-qrcode.min.js";script.async = true;script.onload = () => setIsHtml5QrcodeLoaded(true);document.head.appendChild(script);}}, []);const showToast = (type = 'success') => {setToast({ visible: true, type });setTimeout(() => setToast(prev => ({ ...prev, visible: false })), 2000);};useEffect(() => {let savedSettings = null;try {if (localStorage.getItem('memorabilia_theme') === 'dark') setDarkMode(true);const sItems = localStorage.getItem('memorabilia_items');if (sItems) setItems(JSON.parse(sItems));  const sSett = localStorage.getItem('memorabilia_settings'); 
  if (sSett) { 
...
}, [settings?.lastfmUser, settings?.lastfmApiKey, isLoaded]);useEffect(() => {if (!settings?.lastfmUser || !settings?.lastfmApiKey || !isLoaded || lfmStatIdx === 0) return;const p = LFM_PERIODS[lfmPeriodIdx]; 
