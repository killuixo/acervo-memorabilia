...
// ==========================================
// O link da sua imagem. Sem tokens, limpo, a apontar para o GitHub público.
...
const MButton = ({ onClick, children, className = '', variant = 'primary', icon, darkMode, disabled = false }) => {
  ...
  if (variant === 'green') bgClass = darkMode ? 'bg-emerald-700 text-white' : 'bg-emerald-400 text-black';
          ...
          <MContainer darkMode={darkMode} className="p-4 flex flex-col" colorClass={darkMode ? 'bg-gray-900' : 'bg-white'}>
            ...
            {isBookOrGame && (
              <div className="mb-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  {STATUS_OPTIONS.map(opt => (
                    <button key={opt} onClick={() => setFormData({...formData, status: opt})} className={`px-2 py-1.5 text-[9px] font-bold uppercase tracking-wider border-[4px] shadow-[2px_2px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:translate-x-0.5 active:shadow-none transition-all ${formData.status === opt ? (darkMode ? 'bg-emerald-700 border-emerald-500 text-white shadow-[2px_2px_0px_rgba(100,100,100,0.5)]' : 'bg-emerald-400 border-black text-black') : (darkMode ? 'bg-gray-900 border-gray-600 text-gray-400 shadow-[2px_2px_0px_rgba(100,100,100,0.5)]' : 'bg-white border-black text-black')}`}>{opt}</button>
                  ))}
                </div>
              </div>
            )}
...
const SettingsTab = ({ items, setItems, settings, setSettings, darkMode, setDarkMode, onShowToast, pwa }) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-20 pr-1 relative">
      ...
      {/* NOVO: Botão de Instalação do PWA Renderizado Aqui! */}
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-gray-800 text-gray-200' : 'bg-gray-100 text-black'} font-sans antialiased transition-colors duration-300 select-none`}>
      <div className={`max-w-md mx-auto h-screen relative flex flex-col shadow-2xl overflow-hidden ${darkMode ? 'border-x-[4px] border-gray-600 bg-gray-900' : 'border-x-[4px] border-black bg-white'}`}>
        ...
        <header className={`flex-none p-3 border-b-[4px] z-20 flex justify-between items-center ${darkMode ? 'border-gray-600 bg-gray-900' : 'border-black bg-white'}`}>
          ...
          {/* ============================================================== */}
          {/* LOGO LIMPO (Sem fundo, sem bordas, apenas a imagem)          */}
          {/* ============================================================== */}
        ...
        <main className="flex-1 overflow-hidden p-3 relative z-0">
          ...
          {activeTab === 'settings' && <SettingsTab items={items} setItems={setItems} settings={settings} setSettings={setSettings} darkMode={darkMode} setDarkMode={setDarkMode} onShowToast={() => setGlobalToast(true)} pwa={pwa} />}
        </main>
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-gray-800 text-gray-200' : 'bg-gray-100 text-black'} font-sans antialiased transition-colors duration-300 select-none`}>
      <div className={`max-w-md mx-auto h-screen relative flex flex-col shadow-2xl overflow-hidden ${darkMode ? 'border-x-[4px] border-gray-600 bg-gray-900' : 'border-x-[4px] border-black bg-white'}`}>
        ...
        <header className={`flex-none p-3 border-b-[4px] z-20 flex justify-between items-center ${darkMode ? 'border-gray-600 bg-gray-900' : 'border-black bg-white'}`}>
          ...
          {/* ============================================================== */}
          {/* O LOGO AGORA É APENAS A IMAGEM, SEM BORDA OU QUADRADO        */}
          {/* ============================================================== */}
          <div className="w-12 h-12 flex-shrink-0 flex items-center justify-center">
            <img src={LINK_DO_ICONE_NO_GITHUB} alt="Logo" className="w-full h-full object-contain drop-shadow-md" />
          </div>
