                ...
                <MContainer darkMode={darkMode} className="flex-1 flex flex-row p-2 rounded-r-sm" colorClass={darkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}>
                 ...
                 <div className={`w-24 sm:w-28 flex-shrink-0 flex flex-col items-center justify-between border-l-[3px] pl-2 py-0.5 ${darkMode ? 'border-gray-600' : 'border-gray-300'}`}>
                    ...
                    <div className="flex flex-nowrap justify-center items-center gap-0.5 pointer-events-auto w-full" onClick={e => e.stopPropagation()}>
                       {item.rating === 5 ? (
                         <div 
                           title="Obra-Prima! (Clique para redefinir a nota)"
                           className="cursor-pointer active:scale-90 transition-transform flex items-center justify-center"
                           onClick={() => updateRatingList(item.id, 0)}
                           style={{ 
                             animation: `titleColorCycle ${Math.max(3, (settings?.marqueeSpeed || 35) * 0.25)}s linear infinite`,
                             filter: (settings?.marqueeBrightness ?? 50) > 0 ? `drop-shadow(0 0 ${(settings?.marqueeBrightness ?? 50) * 0.1}px currentColor) drop-shadow(0 0 ${(settings?.marqueeBrightness ?? 50) * 0.15}px currentColor)` : 'none',
                             color: '#f472b6'
                           }}
                         >
                           <Star className="w-[24px] h-[24px] sm:w-[28px] sm:h-[28px] fill-current" />
                         </div>
                       ) : (
