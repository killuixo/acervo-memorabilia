...
// ==========================================
// 2. SISTEMA DE ÁUDIO 8-BIT (SUTIL E OTIMIZADO)
// ==========================================
...
const playChipBeep = (type) => {
  try {
    ...
    if (type === 'success') {
      ...
      // SALVAR: "Blip-Blip" suave e discreto (confirmação final)
      oscillator.type = 'triangle';
...
// ==========================================
// 3. COMPONENTES UI COMPARTILHADOS & MODAIS
// ==========================================
...
// Modal Customizado para evitar alerts nativos que quebram apps mobile/iframes
...
const AddTab = ({ items, setItems, settings, darkMode, addMode, setAddMode, setActiveTab }) => {
    ...
    if (addMode === 'camera_ai' || addMode === 'barcode') {
      ...
      if (addMode === 'barcode') {
        ...
        if (window.Html5Qrcode) {
          ...
          // Prevenção contra múltiplos scripts
          if (!document.getElementById('html5-qrcode-script')) {
            ...
            // Se já existe mas a library não carregou, tentamos novamente após delay
  ...
  const fetchMultiDatabase = async (barcode) => {
    ...
    try {
      ...
      // 2. TENTA OPENLIBRARY (Clássicos, acadêmicos e antigos)
      ...
      if (olData[`ISBN:${barcode}`]) {
        ...
        handleScanSuccess({
          ...
          cover_url: `https://covers.openlibrary.org/b/isbn/${barcode}-L.jpg` // -L para capa grande
        });
        return; 
      }

      // 3. TENTA MUSICBRAINZ (CDs, Vinis, Fitas)
    ...
    try {
      const payload = {
        contents: [{
          role: "user",
          parts: [
            { text: `Extraia as informações desta mídia (foto de capa, ficha, etc). Responda APENAS com JSON no formato exato: 'type' (escolha obrigatoriamente UM entre: ${ALL_TYPES.join(', ')}), 'title', 'author_developer', 'year', 'publisher', 'description' (crie uma breve sinopse de 2 linhas baseada no item).` },
      ...
      if (aiText) {
        // Limpa possíveis marcações markdown do retorno JSON
  ...
  const handleSave = () => {
    if (!formData.title) {
      setScanStatus({ type: 'error', message: 'AVISO: O Título é obrigatório para salvar na biblioteca.' });
      playChipBeep('error');
      return;
    }
        ...
        <MContainer darkMode={darkMode} className="flex-1 mb-4 flex flex-col relative overflow-hidden bg-black items-center justify-center">
          {!hasCameraPermission && !loadingAi && (
            <div className="text-white text-xs font-bold uppercase p-6 text-center leading-relaxed">
              Permissão de câmera não concedida ou bloqueada pelo navegador. <br/><br/>
      ...
      {addMode === 'manual' && (
        <div className="flex-1 overflow-y-auto scrollbar-hide pr-1">
          ...
          <MContainer darkMode={darkMode} className="p-4 flex flex-col" colorClass={darkMode ? 'bg-gray-900' : 'bg-white'}>
            ...
            <MButton darkMode={darkMode} onClick={handleSave} variant="black" className="mt-2 py-4 text-sm"><Check className="w-5 h-5 mr-2" /> Salvar na Biblioteca</MButton>
          </MContainer>
        </div>
      )}
    </div>
