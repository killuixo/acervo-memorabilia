...
const MondrianHBar = ({ label, value, max, index, darkMode, valueFormatter = (v)=>v }) => (
  <div className="flex items-center gap-2 w-full mb-2">
    ...
    <div className={`flex-1 h-5 border-[2px] ${darkMode ? 'bg-gray-800 border-gray-300 shadow-[2px_2px_0px_rgba(209,213,219,1)]' : 'bg-gray-200 border-black shadow-[2px_2px_0px_rgba(0,0,0,1)]'} flex relative overflow-hidden`}>
...
const LibraryTab = ({ items, setItems, darkMode, settings, onShowToast, activeCategories }) => {
  ...
  const baseFilteredItems = useMemo(() => {
      ...
      if (alphaFilter !== 'Todos') {
          result = result.filter(i => {
              ...
              if (sortBy === 'author' && !isVariousArtists(i.author_developer)) {
                  targetText = applyArtistAlias(i.author_developer, settings?.artistAliases);
              }
      ...
      return result;
  }, [items, searchTerm, alphaFilter, sortBy, settings?.artistAliases]);
  ...
  const finalProcessedItems = useMemo(() => {
      ...
      result = [...result].sort((a, b) => {
          ...
          switch (sortBy) {
              case 'title':
                  ...
                  break;
              case 'author':
                  const aAuth = isVariousArtists(a.author_developer) ? a.title : applyArtistAlias(a.author_developer, settings?.artistAliases);
                  const bAuth = isVariousArtists(b.author_developer) ? b.title : applyArtistAlias(b.author_developer, settings?.artistAliases);
                  valA = getSortableName(aAuth);
                  valB = getSortableName(bAuth);
                  break;
              case 'type':
                  valA = (a.type||'').trim();
                  valB = (b.type||'').trim();
      ...
      return result;
  }, [baseFilteredItems, activeFilters, sortBy, sortOrder, settings?.artistAliases]);
      ...
      <div className="flex flex-col h-full pb-20 relative max-w-4xl mx-auto w-full">
        ...
        <div className="flex-1 overflow-y-auto px-1 space-y-4 pb-10 scrollbar-hide">
          ...
          <div className="flex gap-2 flex-col sm:flex-row">
            ...
            {isDiscItem && (
               <a href={`https://open.spotify.com/search/${encodeURIComponent((editedItem.title || '') + ' ' + (editedItem.author_developer || ''))}`} target="_blank" rel="noopener noreferrer" className={`flex-1 p-3 border-[2px] ${darkMode ? 'shadow-[2px_2px_0px_rgba(209,213,219,1)] bg-gray-800 border-gray-300 text-cyan-400' : 'shadow-[2px_2px_0px_rgba(0,0,0,1)] bg-cyan-100 border-black text-cyan-800'} flex items-center justify-center gap-2 font-black uppercase tracking-widest text-[10px] transition-all active:translate-y-1 active:translate-x-1 active:shadow-none`}>
          ...
          <MContainer darkMode={darkMode} className="p-4" colorClass={darkMode ? 'bg-pink-900/20 text-white' : 'bg-pink-50 text-black'}>
            <div className={`flex justify-between items-center mb-3 border-b-[2px] pb-1 ${darkMode ? 'border-gray-300' : 'border-black'}`}>
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      {contextMenuItem && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => setContextMenuItem(null)}>
          <MContainer darkMode={darkMode} className="w-full max-w-xs p-0 flex flex-col overflow-hidden animate-in zoom-in duration-200" colorClass={darkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'} onClick={(e) => e.stopPropagation()}>
             <div className={`p-4 border-b-[2px] ${darkMode ? 'border-gray-300 bg-gray-800' : 'border-black bg-gray-100'} flex justify-between items-center`}>
             ...
             <div className="flex flex-col">
                ...
                <button onClick={() => { window.open(`https://open.spotify.com/search/${encodeURIComponent((contextMenuItem.title||'')+' '+(contextMenuItem.author_developer||''))}`, '_blank'); setContextMenuItem(null); }} className={`p-4 flex items-center gap-3 text-[11px] font-black uppercase tracking-widest border-b-[2px] ${darkMode ? 'border-gray-700 hover:bg-cyan-900/30' : 'border-gray-200 hover:bg-cyan-50'} transition-colors text-cyan-600 dark:text-cyan-600 text-left`}><Headphones className="w-5 h-5" /> Ouvir (Spotify)</button>
                <button onClick={() => { window.open(`https://www.discogs.com/search?q=${contextMenuItem.barcode||encodeURIComponent((contextMenuItem.title||'')+' '+(contextMenuItem.author_developer||''))}&type=all`, '_blank'); setContextMenuItem(null); }} className={`p-4 flex items-center gap-3 text-[11px] font-black uppercase tracking-widest border-b-[2px] ${darkMode ? 'border-gray-700 hover:bg-amber-900/30' : 'border-gray-200 hover:bg-amber-50'} transition-colors text-amber-600 dark:text-amber-500 text-left`}><DiscIcon className="w-5 h-5" /> Buscar Preço (Discogs)</button>
      ...
      {isFilterMenuOpen && (
          <div className="fixed inset-0 z-[999] bg-black/80 flex justify-center items-end sm:items-center animate-in fade-in duration-200">
              <div className={`w-full sm:max-w-md max-h-[85vh] sm:h-[80vh] flex flex-col border-t-[2px] sm:border-[2px] ${darkMode ? 'bg-gray-900 border-gray-300 shadow-[6px_6px_0px_rgba(209,213,219,1)]' : 'bg-white border-black shadow-[6px_6px_0px_rgba(0,0,0,1)]'}`}>
                  <div className={`p-4 border-b-[2px] flex justify-between items-center ${darkMode ? 'border-gray-300' : 'border-black'}`}>
                  ...
                  <div className="flex-1 overflow-y-auto scrollbar-hide">
                      {Object.entries(FILTER_OPTIONS).map(([category, options]) => {
                          ...
                          return (
                              <div key={category} className={`border-b-[2px] ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
                                  <button
                                              ...
                                              return (
                                                  <div key={option} onClick={() => handleCheckboxChange(category, option)} className={`flex items-center justify-between p-3 cursor-pointer transition-colors active:bg-black/5 ${darkMode ? 'border-t border-gray-800' : 'border-t border-gray-200'}`}>
                                                      ...
                                                      <div className="flex items-center gap-3">
                                                          ...
                                                          <div className={`w-5 h-5 border-[2px] flex items-center justify-center flex-shrink-0 ${isChecked ? (darkMode ? 'border-cyan-400 bg-cyan-900/40' : 'border-cyan-600 bg-cyan-100') : (darkMode ? 'border-gray-500' : 'border-gray-400')}`}>
      ...
      <div className={`sticky top-0 z-20 flex flex-col gap-2 pb-2 pt-1 border-b-[2px] ${darkMode ? 'bg-gray-900 border-gray-700' : 'bg-gray-100 border-gray-300'} mb-3 px-1`}>
          ...
          <div className="flex gap-1 sm:gap-2 w-full">
              <div className="flex-1 relative min-w-0">
                  ...
                  <input
                      ...
                      className={`w-full h-10 pl-8 pr-8 border-[2px] font-black text-[10px] sm:text-[11px] uppercase tracking-wider outline-none transition-all focus:border-cyan-700 ${darkMode ? 'bg-gray-800 text-white border-gray-400 shadow-[2px_2px_0px_rgba(209,213,219,1)]' : 'bg-white text-black border-black shadow-[2px_2px_0px_rgba(0,0,0,1)]'}`}
                  />
                  {searchTerm && (
              ...
              <button
                  ...
                  className={`w-10 h-10 flex-shrink-0 flex items-center justify-center border-[2px] transition-all active:translate-y-0.5 active:translate-x-0.5 active:shadow-none relative ${darkMode ? 'bg-gray-800 text-white border-gray-400 shadow-[2px_2px_0px_rgba(209,213,219,1)]' : 'bg-white text-black border-black shadow-[2px_2px_0px_rgba(0,0,0,1)]'}`}
              ...
              <select
                  ...
                  className={`w-14 sm:w-16 h-10 p-0 text-center text-[10px] font-black uppercase tracking-widest border-[2px] outline-none cursor-pointer flex-shrink-0 ${darkMode ? 'border-gray-400 shadow-[2px_2px_0px_rgba(209,213,219,1)] bg-gray-800 text-white' : 'border-black shadow-[2px_2px_0px_rgba(0,0,0,1)] bg-white text-black'}`}
              >
                ...
                <MContainer darkMode={darkMode} className="flex-1 flex flex-row p-2 rounded-r-sm" colorClass={darkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}>
                  ...
                  <div className="flex-1 flex flex-col justify-between pr-3 pointer-events-none">
                    ...
                    <div className="mt-3 flex items-end">
                      {[...(activeCategories['Livros'] || []), ...(activeCategories['Games'] || [])].includes(item.type) ? <div className={`text-[8px] px-2 py-1 border-[2px] ${darkMode ? 'border-gray-300 bg-cyan-900 text-cyan-300' : 'border-black bg-amber-500 text-white'} font-black uppercase tracking-widest w-max`}>{item.status || '--'}</div> : <div />}
                    </div>
                 </div>
...
const DashboardTab = ({ items, darkMode, activeCategories, settings }) => {
  ...
  const byAuthor = dashItems.reduce((acc, i) => {
    if (i.author_developer) {
      ...
      if (!acc[normAuthor]) acc[normAuthor] = { display: mappedAuthor, count: 0 };
      acc[normAuthor].count += 1;
    }
    return acc;
  }, {});

  const sortedAuthors = Object.entries(byAuthor).map(([key, data]) => [data.display, data.count]).sort((a, b) => b[1] - a[1]).slice(0, 5);
     ...
     discsWithTracks.forEach(i => {
         ...
         if (i.author_developer) {
            ...
            if (!isVariousArtists(rawAuthor)) {
               const mappedAuthor = applyArtistAlias(rawAuthor, settings?.artistAliases);
               let auth = getSortableName(mappedAuthor);
  ...
  }, [videoItems, hasVideoStats, settings?.artistAliases]);
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-20 pr-1 space-y-4 scrollbar-hide max-w-5xl mx-auto w-full">
      ...
      {hasMusicStats && (
        <MContainer darkMode={darkMode} className="p-4" colorClass={darkMode ? 'bg-amber-900/40 text-white' : 'bg-amber-50 text-black'}>
           <div className={`text-[10px] font-black uppercase tracking-widest mb-4 border-b-[2px] pb-2 flex items-center gap-2 ${darkMode ? 'border-gray-300' : 'border-black'}`}><Music className="w-4 h-4" /> Auditoria Musical (Discos no Filtro: {musicItems.length})</div>
      ...
      {hasVideoStats && (
        <MContainer darkMode={darkMode} className="p-4" colorClass={darkMode ? 'bg-pink-900/40 text-white' : 'bg-pink-50 text-black'}>
           <div className={`text-[10px] font-black uppercase tracking-widest mb-4 border-b-[2px] pb-2 flex items-center gap-2 ${darkMode ? 'border-gray-300' : 'border-black'}`}><MonitorPlay className="w-4 h-4" /> Auditoria Cinematográfica (Filmes: {videoItems.length})</div>
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {yearsKeys.length > 0 && (
              <MContainer darkMode={darkMode} className="p-4 flex flex-col md:col-span-2" colorClass={darkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'}>
                <div className={`text-[10px] font-black uppercase tracking-widest mb-4 border-b-[2px] pb-2 flex justify-between ${darkMode ? 'border-gray-300' : 'border-black'}`}><span>Linha do Tempo (Exata)</span><Calendar className="w-4 h-4" /></div>
...
const SettingsTab = ({ items, setItems, settings, setSettings, darkMode, setDarkMode, onShowToast, pwa, activeCategories, activeClassCodes }) => {
  ...
  const handleAddAlias = () => {
    if (!newAlias.main || !newAlias.alias) {
      playChipBeep('error');
      onShowToast('error');
      return;
    }
    const updatedAliases = [...(settings?.artistAliases || []), { main: newAlias.main.trim(), alias: newAlias.alias.trim() }];
    setSettings({ ...settings, artistAliases: updatedAliases });
    setNewAlias({ main: '', alias: '' });
    playChipBeep('save');
    onShowToast('success');
  };

  const handleRemoveAlias = (index) => {
    const updatedAliases = [...(settings?.artistAliases || [])];
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-20 pr-1 relative max-w-3xl mx-auto w-full">
          ...
          <div className="p-4 flex flex-col gap-4">
            ...
            <div className={`border-t-[2px] ${darkMode ? 'border-amber-900' : 'border-amber-200'} pt-3 flex flex-col gap-5`}>
               <div>
                 ...
                 <input type="range" min="10" max="150" step="1" value={160 - (Number(settings?.marqueeSpeed) || 35)} onChange={e => setSettings({...settings, marqueeSpeed: 160 - parseInt(e.target.value)})} onMouseUp={() => { playChipBeep('save'); onShowToast('success'); }} onTouchEnd={() => { playChipBeep('save'); onShowToast('success'); }} className={`w-full h-2 rounded-lg cursor-pointer ${darkMode ? 'bg-gray-700' : 'bg-gray-300'}`} style={{ accentColor: '#0891b2' }} />
               </div>
               <div>
                 ...
                 <input type="range" min="0" max="100" step="5" value={Number(settings?.marqueeBrightness) ?? 50} onChange={e => setSettings({...settings, marqueeBrightness: parseInt(e.target.value)})} onMouseUp={() => { playChipBeep('save'); onShowToast('success'); }} onTouchEnd={() => { playChipBeep('save'); onShowToast('success'); }} className={`w-full h-2 rounded-lg cursor-pointer ${darkMode ? 'bg-gray-700' : 'bg-gray-300'}`} style={{ accentColor: '#d97706' }} />
               </div>
            </div>
          </div>
        )}
                ...
                <div className="flex flex-col gap-2">
                  {settings.artistAliases.map((a, idx) => (
                    <div key={idx} className={`flex items-center justify-between p-2 border-[2px] ${darkMode ? 'border-gray-600 bg-gray-800' : 'border-gray-300 bg-white'}`}>
                      <div className="flex flex-col">
                        <span className="text-[9px] font-black uppercase tracking-widest opacity-60">Lido como: {a.alias}</span>
                        <span className="text-[11px] font-bold">Corrigido p/: {a.main}</span>
                      </div>
      ...
      <div className="mt-8 mb-4 text-center">
        <button onClick={() => setShowResetConfirm(true)} className={`px-4 py-2 border-[2px] text-[8px] font-black uppercase tracking-widest opacity-60 hover:opacity-100 transition-all ${darkMode ? 'border-pink-500 text-pink-500' : 'border-pink-600 text-pink-600'}`}>
...
4. Retorne APENAS o JSON puro. Nenhuma palavra a mais.`;
  ...
  const renderMarqueeContent = () => {
    const statsArr = [];
    statsArr.push(<span key="total" className={`text-white ${ledItemStyle}`}>ACERVO TOTAL: {totalItens}</span>);
      ...
      <div className={`w-full h-screen relative flex flex-col md:flex-row shadow-2xl overflow-hidden ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
        ...
        <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
          ...
          <main className="flex-1 overflow-hidden p-0 sm:p-2 lg:p-6 relative flex flex-col">
            ...
            {activeTab === 'dashboard' && <div className="p-3 overflow-y-auto w-full"><DashboardTab items={items} darkMode={darkMode} activeCategories={activeCategories} settings={settings} /></div>}
