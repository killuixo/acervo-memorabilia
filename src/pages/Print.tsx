import React, { useState, useEffect, useMemo } from 'react';import { useNavigate } from 'react-router-dom';// Ícones minimalistas (SVGs puros com traços rígidos)const PrinterIcon = () => ();const SettingsIcon = () => ();interface CatalogItemType {ID?: string;'Localização'?: string;'Título'?: string;'Autor/Desenvolvedor'?: string;'Status'?: string;'Nota'?: string;'Código Arquivístico'?: string;'Código de Barras'?: string;'Tipo'?: string;'Ano'?: string;'Editora/Gravadora'?: string;'Páginas/Tempo'?: string;'Descrição'?: string;'Anotações'?: string;'URL da Capa'?: string;[key: string]: any;}function parseCSV(str: string): string[][] {const result: string[][] = [];let row: string[] = [];let col = '';let inQuotes = false;for (let i = 0; i < str.length; i++) {let char = str[i];let nextChar = str[i + 1];if (char === '"') {
...
}if (col !== '' || row.length > 0) {row.push(col);result.push(row);}return result;}function getInitial(text: string): string {if (!text) return '#';const clean = text.normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().charAt(0).toUpperCase();if (!/^[A-Z]$/.test(clean)) return '#';return clean;}function getSortKey(item: CatalogItemType): string {let author = (item['Autor/Desenvolvedor'] || '').trim();let title = (item['Título'] || '').trim();const authorLower = author.toLowerCase();if (!author || authorLower === 'various' || authorLower === 'vários' || authorLower === 'various artists') {return title;}return author;}function getMondrianPattern(id?: string | number) {const colors = ['#ff0055', '#00ccff', '#ffcc00', '#ffffff', '#ffffff'];let hash = 0;const safeId = id ? String(id) : 'default';for (let i = 0; i < safeId.length; i++) {hash = safeId.charCodeAt(i) + ((hash << 5) - hash);}const rand = () => {const x = Math.sin(hash++) * 10000;return x - Math.floor(x);};const splitX = 25 + Math.floor(rand() * 50);const splitY = 25 + Math.floor(rand() * 50);const itemColors: string[] = [];for (let i = 0; i < 4; i++) {itemColors.push(colors[Math.floor(rand() * colors.length)]);}return { splitX, splitY, itemColors };}const CoverImage = ({ src, id }: { src?: string; id?: string | number }) => {const [hasError, setHasError] = useState(!src);useEffect(() => {setHasError(!src);}, [src]);const { splitX, splitY, itemColors } = useMemo(() => getMondrianPattern(id), [id]);if (hasError) {return (<divclassName="w-full aspect-square border-[2px] border-[#111] grid bg-[#111] gap-[2px] overflow-hidden"style={{gridTemplateColumns: ${splitX}fr ${100 - splitX}fr,gridTemplateRows: ${splitY}fr ${100 - splitY}fr}}><div style={{ backgroundColor: itemColors[0] }}><div style={{ backgroundColor: itemColors[1] }}><div style={{ backgroundColor: itemColors[2] }}><div style={{ backgroundColor: itemColors[3] }}>);}return (<imgsrc={src}alt="Capa"onError={() => setHasError(true)}className="w-full aspect-square object-cover border-[2px] border-[#111] bg-white"/>);};const RatingStars = ({ nota }: { nota?: string }) => {if (nota === undefined || nota === null || nota === '') return null;const num = parseInt(nota, 10);if (isNaN(num)) return null;if (num === 0) return ☆;if (num >= 5) {return (<spanclassName="text-[14pt] font-black leading-none drop-shadow-[1px_1px_0_#111] mt-1"style={{background: 'linear-gradient(45deg, #ff0055, #00ccff, #ffcc00)',WebkitBackgroundClip: 'text',WebkitTextFillColor: 'transparent',display: 'inline-block'}}>★);}let color = "text-[#ffcc00]";if (num === 3) color = "text-[#00ccff]";if (num === 4) color = "text-[#ff0055]";return (<span className={${color} font-black text-[9pt] leading-none drop-shadow-[1px_1px_0_#111] tracking-widest mt-1}>{'★'.repeat(num)});};const CatalogItem = ({ item }: { item: CatalogItemType }) => {const isCD = item['Tipo'] && String(item['Tipo']).trim().toUpperCase() === 'CD';return (<CoverImage src={item['URL da Capa']} id={item.ID} />ID: {item.ID || '-'}LOC: {item['Localização'] || '-'}  <div className="flex flex-col flex-1 overflow-hidden">
...
);};const CatalogPage = ({ items, pageNum, totalPages, date, userName }: { items: CatalogItemType[]; pageNum: number; totalPages: number; date: string; userName: string }) => {const firstItem = items[0] || {};const lastItem = items[items.length - 1] || {};const sortKeyFirst = getSortKey(firstItem) || '?';const sortKeyLast = getSortKey(lastItem) || '?';const initialA = getInitial(sortKeyFirst);const initialB = getInitial(sortKeyLast);const dictHeader = initialA === initialB ? initialA : ${initialA} — ${initialB};return ({dictHeader}MEMORABILIA  <div className="grid grid-cols-2 gap-x-6 gap-y-4 flex-1 items-start content-start">
    {items.map((item, idx) => (
...
);};export default function PrintApp() {const navigate = useNavigate();const [data, setData] = useState<CatalogItemType[]>([]);const [loading, setLoading] = useState(false);const [showTip, setShowTip] = useState(false);const [printError, setPrintError] = useState(false);const [showSettings, setShowSettings] = useState(false);const [userName, setUserName] = useState('');const handleFileUpload = (e: React.ChangeEvent) => {const file = e.target.files?.[0];if (!file) return;setLoading(true);
...
e.target.value = '';
};const handlePrint = () => {try {window.print();} catch (error) {setPrintError(true);setTimeout(() => setPrintError(false), 5000);}};// --- LÓGICA DE EXPORTAÇÃO PARA BLOGGER ---const handleExportHTML = () => {// Helper para as estrelas no HTML geradoconst getStaticRatingHTML = (nota: string | undefined) => {if (!nota) return '';const num = parseInt(nota, 10);if (isNaN(num)) return '';if (num === 0) return <span class="text-[#111] text-[10pt] font-black mt-1">☆</span>;if (num >= 5) {return <span class="text-[14pt] font-black leading-none drop-shadow-[1px_1px_0_#111] mt-1" style="background: linear-gradient(45deg, #ff0055, #00ccff, #ffcc00); -webkit-background-clip: text; -webkit-text-fill-color: transparent; display: inline-block;">★</span>;}let color = "text-[#ffcc00]";if (num === 3) color = "text-[#00ccff]";if (num === 4) color = "text-[#ff0055]";return <span class="${color} font-black text-[9pt] leading-none drop-shadow-[1px_1px_0_#111] tracking-widest mt-1">${'★'.repeat(num)}</span>;};// Helper para gerar o HTML de cada item
...
};const ITEMS_PER_PAGE = 10;const pages = useMemo(() => {const chunks = [];for (let i = 0; i < data.length; i += ITEMS_PER_PAGE) {chunks.push(data.slice(i, i + ITEMS_PER_PAGE));}return chunks;}, [data]);const currentDate = new Date().toLocaleDateString('pt-BR');return (<style dangerouslySetInnerHTML={{__html: `@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;700;900&display=swap');    body { font-family: 'Inter', system-ui, sans-serif; }
  ...
  {data.length === 0 ? (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-white border-[4px] border-[#111] max-w-lg w-full p-10 relative shadow-[10px_10px_0px_0px_rgba(17,17,17,1)]">
        ...
        {/* BOTÃO DE VOLTAR NA TELA INICIAL DO PRINT */}
        <div className="mt-4 flex justify-center z-20 relative">
           <button onClick={() => navigate('/')} className="font-black text-[10px] uppercase tracking-widest text-[#111] underline hover:text-[#ff0055] transition-colors">
              &lt; Voltar ao Acervo Digital
           </button>
        </div>
    ...
    <div className="pb-20">
      <div className="no-print sticky top-0 bg-white border-b-[4px] border-[#111] p-4 flex flex-col sm:flex-row justify-between items-center z-50 shadow-md gap-4">
        ...
        <div className="flex flex-wrap justify-center sm:justify-end items-center gap-2 sm:gap-3">
          {/* BOTÃO DE VOLTAR NO CABEÇALHO (QUANDO JÁ TEM ARQUIVO CARREGADO) */}
          <button 
            onClick={() => navigate('/')}
